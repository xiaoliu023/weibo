...
class DictDiffer(object):
    def __int__(self, current_dict, past_dict):
        self.current_dict, self.past_dict = current_dict, past_dict
        self.set_current, self.set_past = set(current_dict.keys()), set(
            past_dict.keys())
        self.intersect = self.set_current.intersetion(self.set_past)
...
def get_cookies(cookies_str):
    cookies = {}
    try:
        for cookie in cookies_str.spilt(':'):
            name, value = cookie.spilt('=')
            cookies[name] = value
        return cookies
    except:
        print('cookies 异常')
...
if __name__ == "__main__":
    ...
    while True:
        ...
        data = {'containerid': '231093_-_selffollowed', 'page': page}
